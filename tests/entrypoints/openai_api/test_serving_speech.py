...
def test_rolling_icl_generate_audio_bytes_routes_to_orchestrator(mocker: MockerFixture):
    """Long-form Raon requests should route through the rolling orchestrator."""
    server = OmniOpenAIServingSpeech.__new__(OmniOpenAIServingSpeech)
    server._tts_model_type = "raon"
    server._diffusion_mode = False

    # Force the Raon routing gate on without touching ENV.
    mocker.patch(
        "vllm_omni.model_executor.models.raon.serving_utils.should_use_rolling_icl",
        return_value=True,
    )

    pcm = np.zeros(2400, dtype=np.float32)
    orchestrator = AsyncMock(return_value=(pcm, 24000))
    mocker.patch(
        "vllm_omni.model_executor.models.raon.serving_utils.generate_raon_long_tts_rolling_icl",
        orchestrator,
    )
    # The one-shot path must not run once rolling routing is active.
    mocker.patch.object(
        OmniOpenAIServingSpeech,
        "_prepare_speech_generation",
        AsyncMock(side_effect=AssertionError("one-shot path must not run when rolling_icl is active")),
    )

    fake_response = MagicMock()
    fake_response.audio_data = b"PCMBYTES"
    fake_response.media_type = "audio/wav"
    mocker.patch.object(OmniOpenAIServingSpeech, "create_audio", return_value=fake_response)
    ...
    audio_data, media_type = asyncio.run(server._generate_audio_bytes(request))
    orchestrator.assert_awaited_once_with(server, request)
    assert audio_data == b"PCMBYTES"
    assert media_type == "audio/wav"


def test_rolling_icl_generate_audio_bytes_skips_for_short_input(mocker: MockerFixture):
    """Short Raon requests should stay on the regular one-shot path."""
    server = OmniOpenAIServingSpeech.__new__(OmniOpenAIServingSpeech)
    server._tts_model_type = "raon"
    server._diffusion_mode = False
    ...
    orchestrator = AsyncMock(side_effect=AssertionError("orchestrator must not run when gating returns False"))
    mocker.patch(
        "vllm_omni.model_executor.models.raon.serving_utils.generate_raon_long_tts_rolling_icl",
        orchestrator,
    )
    ...
    mocker.patch.object(
        OmniOpenAIServingSpeech,
        "_prepare_speech_generation",
        AsyncMock(return_value=("speech-id", _fake_generator(), {})),
    )
    mocker.patch.object(
        OmniOpenAIServingSpeech,
        "_extract_audio_output",
        return_value=({"audio": torch.zeros(2400), "sr": 24000}, "audio"),
    )
    ...
    audio_data, _ = asyncio.run(server._generate_audio_bytes(request))
